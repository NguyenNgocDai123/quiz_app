...
class AppResponseModel(BaseModel, Generic[T]):
    ...
    business_code: int | None = Field(
        default=None,
        description="Optional business-specific code for the response (e.g., custom error codes).",
    )
    status_code: int = Field(
        ...,
        description="HTTP status code of the response (e.g., 200 for success, 404 for not found).",
    )
    message: str = Field(
        ..., description="A human-readable message describing the response result."
    )
    data: T = Field(
        default=None,
        description="The actual payload of the response. Can be any type (e.g., object, list, etc.).",
    )
...
class BusinessJsonResponse(JSONResponse):
    ...
    def __init__(
        self, business_code: int, content: Any, status_code: int = 200, **kwargs: Any
    ) -> None:
        """
        Initializes a BusinessJsonResponse with a business code in the headers.

        Args:
            business_code (int): The business code to include in the response headers.
